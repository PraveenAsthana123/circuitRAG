"""
Datadog MCP server — read-only Stage-1 (iter-71 SDLC batch).

Per CLAUDE.md §44 (iter-71; SDLC fleet expansion), §47 (each MCP
server owns ONE namespace; datadog.* is the Datadog boundary), §47.6
(read-only Stage-1; write surfaces deferred — Datadog has no destructive APIs at the agent boundary in Stage-1.).

TOOLS (read only)
  datadog.metric_query    Query a metric over a time range.
  datadog.log_search    Search Datadog logs by query.
...
log = logging.getLogger("mcp.server_datadog")

app = FastAPI(title="DocuMind MCP — Datadog server")
setup_server_otel(app, service_name="mcp-server-datadog")
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "datadog.metric_query",
        "description": 'Query a metric over a time range.',
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "side_effects": "read",
        "required_scopes": ['datadog:read'],
        "idempotent": True,
    },
    {
        "name": "datadog.log_search",
        "description": 'Search Datadog logs by query.',
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "maxLength": 500},
...
def _metric_query_impl(args: dict[str, Any]) -> dict[str, Any]:
    if "query" in args:
...
def _log_search_impl(args: dict[str, Any]) -> dict[str, Any]:
    if "query" in args:
        _validate_query(str(args.get("query", "")))
    live, reason = _live_or_stub()
    if not live:
...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "mcp-server-datadog"}
...
async def _dispatch(req: ToolCallRequest, idempotency_key: str | None, cid: str) -> dict[str, Any]:
    ...
    try:
        if req.name == "datadog.metric_query":
            return _metric_query_impl(req.arguments)
        if req.name == "datadog.log_search":
            return _log_search_impl(req.arguments)
