...
async def run() -> int:
    ...
    async with async_playwright() as p:
        ...
        # Re-check: still no overlay popup after selection.
        ...
        if sel_btn >= 1 and overlay_after_sel == 0:
            ok("step 7: ≥3-word selection shows 'Read selection' button — no popup spawned")
        elif sel_btn < 1:
            fail("step 7: 'Read selection' button never appeared on selection")
            failures += 1
        else:
