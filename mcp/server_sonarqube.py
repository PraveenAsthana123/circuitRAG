"""
SonarQube MCP server — read-only Stage-1 (iter-71 SDLC batch).

Per CLAUDE.md §44 (iter-71; SDLC fleet expansion), §47 (each MCP
server owns ONE namespace; sonarqube.* is the SonarQube boundary), §47.6
(read-only Stage-1; write surfaces deferred — Quality-gate writes (mark issue resolved/false-positive) are write surface.).

TOOLS (read only)
  sonarqube.issues_search    Search code-quality issues by project/severity/type.
  sonarqube.measures_get    Get project measures (coverage, duplications, debt).
...
log = logging.getLogger("mcp.server_sonarqube")

app = FastAPI(title="DocuMind MCP — SonarQube server")
setup_server_otel(app, service_name="mcp-server-sonarqube")
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "sonarqube.issues_search",
        "description": 'Search code-quality issues by project/severity/type.',
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "side_effects": "read",
        "required_scopes": ['sonarqube:read'],
        "idempotent": True,
    },
    {
        "name": "sonarqube.measures_get",
        "description": 'Get project measures (coverage, duplications, debt).',
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "maxLength": 500},
...
def _live_or_stub() -> tuple[bool, str]:
    keys = ('SONAR_HOST_URL', 'SONAR_TOKEN')
...
def _issues_search_impl(args: dict[str, Any]) -> dict[str, Any]:
    if "query" in args:
...
def _measures_get_impl(args: dict[str, Any]) -> dict[str, Any]:
    if "query" in args:
        _validate_query(str(args.get("query", "")))
    live, reason = _live_or_stub()
    if not live:
...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "mcp-server-sonarqube"}
...
async def _dispatch(req: ToolCallRequest, idempotency_key: str | None, cid: str) -> dict[str, Any]:
    ...
    try:
        if req.name == "sonarqube.issues_search":
            return _issues_search_impl(req.arguments)
        if req.name == "sonarqube.measures_get":
            return _measures_get_impl(req.arguments)
