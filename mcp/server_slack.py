"""
Slack MCP server — read-only Stage-1 (iter-71 SDLC batch).

Per CLAUDE.md §44 (iter-71; SDLC fleet expansion), §47 (each MCP
server owns ONE namespace; slack.* is the Slack boundary), §47.6
(read-only Stage-1; write surfaces deferred — Sending Slack messages is externally-visible and consent-regulated; needs separate ADR.).

TOOLS (read only)
  slack.channel_list    List channels in the configured workspace.
  slack.message_search    Search Slack messages by query (read-only).
...
log = logging.getLogger("mcp.server_slack")

app = FastAPI(title="DocuMind MCP — Slack server")
setup_server_otel(app, service_name="mcp-server-slack")
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "slack.channel_list",
        "description": 'List channels in the configured workspace.',
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "side_effects": "read",
        "required_scopes": ['slack:read'],
        "idempotent": True,
    },
    {
        "name": "slack.message_search",
        "description": 'Search Slack messages by query (read-only).',
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "maxLength": 500},
...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "mcp-server-slack"}
...
async def _dispatch(req: ToolCallRequest, idempotency_key: str | None, cid: str) -> dict[str, Any]:
    ...
    try:
        if req.name == "slack.channel_list":
            return _channel_list_impl(req.arguments)
        if req.name == "slack.message_search":
