"""
Sentry MCP server — read-only Stage-1 (iter-71 SDLC batch).

Per CLAUDE.md §44 (iter-71; SDLC fleet expansion), §47 (each MCP
server owns ONE namespace; sentry.* is the Sentry boundary), §47.6
(read-only Stage-1; write surfaces deferred — Resolving/ignoring issues is write surface; deferred.).

TOOLS (read only)
  sentry.issue_search    Search Sentry issues by project/level/status.
  sentry.event_lookup    Look up a single event by event_id.
...
log = logging.getLogger("mcp.server_sentry")

app = FastAPI(title="DocuMind MCP — Sentry server")
setup_server_otel(app, service_name="mcp-server-sentry")
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "sentry.issue_search",
        "description": 'Search Sentry issues by project/level/status.',
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "side_effects": "read",
        "required_scopes": ['sentry:read'],
        "idempotent": True,
    },
    {
        "name": "sentry.event_lookup",
        "description": 'Look up a single event by event_id.',
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "maxLength": 500},
...
def _live_or_stub() -> tuple[bool, str]:
    keys = ('SENTRY_AUTH_TOKEN', 'SENTRY_ORG')
...
def _event_lookup_impl(args: dict[str, Any]) -> dict[str, Any]:
    if "query" in args:
        _validate_query(str(args.get("query", "")))
    live, reason = _live_or_stub()
    if not live:
...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "mcp-server-sentry"}
...
async def _dispatch(req: ToolCallRequest, idempotency_key: str | None, cid: str) -> dict[str, Any]:
    ...
    try:
        if req.name == "sentry.issue_search":
            return _issue_search_impl(req.arguments)
        if req.name == "sentry.event_lookup":
            return _event_lookup_impl(req.arguments)
