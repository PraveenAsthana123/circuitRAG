...
| ~~Risk classification (§7) had 4 buckets but no decision tree~~ ✅ Iter 132 (2026-05-20) | ~~P1 local gap~~ closed | `risk/risk-classifier.ts` ships typed `UseCaseDescriptor` → `RiskClassification` mapping per EU AI Act Regulation 2024/1689 (Art. 5 prohibited / Art. 6 + Annex III high-risk / Art. 50 limited-risk / minimal-risk). Decision tree is conservative — automated decisions on PII without explicit Annex III match → high-risk (§57.7 "better over-classify than under" rule). High-risk classification emits Art. 9-17 + 86 obligations + GDPR Art. 8 if minors + HIPAA/etc. layered regulated-data obligations + flags requiresAIActKPIs=true composing with iter 131's `euAiActDisclosureSet()`. 14 drill tests, 6 negatives. |
...
| ~~"Auto-Heal" in §12 was named but unscoped~~ ✅ Iter 134 (2026-05-20) | ~~P1 local gap~~ closed | `failure/auto-heal-catalog.ts` ships 3-bucket partition over iter 133's matrix: `autoHealCatalog()` (system recovers itself — 8 kinds) / `escalationCatalog()` (ops fixes config/credential/scope — 4 kinds) / `hitlCatalog()` (human reviews content/decision — 3 kinds). `bucketFor(kind)` + `isAutoHealable(kind)` + `escalationTargetFor(kind)` operator helpers. §59.1 MDD: catalog DERIVED from iter 133 matrix — single source of truth, future failure kinds flow through automatically. The 7 cardinal NEGATIVES (guardrail / 401 / 403 / data_integrity / hallucination / 4xx / tool_auth_denied are NOT auto-healable) are drilled at the catalog layer too — defense-in-depth against the §57.7 "retry everything" reflex. 22 drill tests, 9 negatives. |
...
| ~~§1–§3 C4 diagrams had no L4 (code) level~~ ✅ Iter 136 (2026-05-20) | ~~P1 local gap~~ closed | `lld/c4-l4-generator.ts` ships typed Mermaid generator: `generateComponentL4(id)` produces per-component L4 (interfaces as boxes, DI seams as trapezoids `[/X/]`, error envelopes as cylinders, composes/depends-on edges) + `generateSystemL4()` produces system-wide composesWith graph. Architecture-introspection helpers: `findOrphans()` (zero — proven by drill), `highFanIn()` / `highFanOut()` (surfaces foundational + aggregator components for §52 brutal review). §59.1 MDD: generator DERIVED from iter 135 LLD registry — diagram regenerates from registry mutations, no hand-edit. 17 drill tests, 2 negatives. |
| ~~§5 LLD table was one-sentence per component~~ ✅ Iter 135 (2026-05-20) | ~~P1 local gap~~ closed | `lld/component-lld.ts` ships typed `ComponentLLD` registry covering all 10 openclaw components with 7 mandatory fields each (description / primaryInterfaces with seam-flagging / errorEnvelopes with failureKind hints / nfrRefs / kpiRefs / composesWith / riskCategories / owner). `validateRegistry()` enforces cross-reference integrity at drill time (every composesWith → real ComponentId, every nfrRef → CANONICAL_FLEET_NFRS id, every kpiRef → CANONICAL_KPIS id, no self-loops). Helpers: `lldFor()`, `componentsThatCompose()` (reverse-dep graph), `allSeams()` (DI seams across registry). 17 drill tests, 2 negatives. Composes with iters 130 (NFR refs) + 131 (KPI refs) + 133 (failureKind hints) + every component refactor iter (DI seams flagged inline). |
...
| ~~§13 Failure Handling Flow had 5 strategies with no when-to-use guidance~~ ✅ Iter 133 (2026-05-20) | ~~P1 local gap~~ closed | `failure/failure-decision-matrix.ts` ships typed `FailureKind` taxonomy (15 named kinds across transient/permanent/safety/quota categories) → `FailureDecision` (primaryStrategy + secondaryStrategy + maxAttempts + initialBackoffMs + recordToAudit + requiresHITL + strideCategory + rationale). `nextAction()` per-attempt engine returns retry/circuit_breaker/fallback_model/escalation/hitl_review/fail_fast with high-risk envelope shortening per Art. 14. Locks 7 NEGATIVE invariants preventing retry-when-you-shouldn't (4xx → fail_fast / 401+403 → escalation / guardrail → HITL / data integrity → fail_fast / etc.). 24 drill tests, 11 negatives. Composes with iters 130 (NFR alerts) + 131 (KPI tracking) + 132 (risk envelope). |
