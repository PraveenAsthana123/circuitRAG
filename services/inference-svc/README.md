...
🧩 **Service**  ·  **Path:** `services/inference-svc`  ·  **Generated:** 2026-05-16 23:25 UTC
...
| go.mod | ❌ |
| package.json | ❌ |
| Top git contributors | `78	PraveenAsthana123`, `6	Praveen` |
...
| 6 | Async I/O where applicable | **10** | 56 async functions detected |
| 7 | Timeouts on all external calls | **10** | ✓ timeout= or asyncio.wait_for — detected at `app/agents/multi_hop_fanout.py:144` |
...
| 2 | Circuit breaker on external deps | **10** | ✓ CircuitBreaker wired — detected at `app/agents/multi_hop_agent.py:28` |
| 3 | Graceful degradation path | TBD | reviewer audit |
| 4 | Health probe (startup/liveness/readiness) | **10** | ✓ `/health` endpoint — detected at `app/routers/__init__.py:52` |
...
| 3 | Structured JSON logs | **10** | ✓ structured logger — detected at `app/main.py:51` |
| 4 | correlation_id propagated everywhere | **10** | ✓ correlation_id used — detected at `app/main.py:368` |
| 5 | Tracing (OTel) wired | **10** | ✓ OTel imported — detected at `app/services/guardrails.py:43` |
| 6 | Metrics exposed (RED: rate/errors/duration) | **10** | ✓ Prometheus instrumentation — detected at `app/routers/__init__.py:146` |
...
| 10 | Decision audit row per AI call (§38+§48) | **10** | ✓ decision_audit ref — detected at `app/main.py:95` |
...
| 2 | Test cases auto-parsed | **10** | 3 test functions |
...
| 10 | Sequence diagrams up-to-date | **10** | 3 endpoints diagrammed |
