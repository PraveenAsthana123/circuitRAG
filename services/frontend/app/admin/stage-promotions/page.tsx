...
const COMPONENTS: Component[] = [
  ...
  // ── Layer scaffolds (different Stage-2/3 patterns) ────────────────────
  {
    ...
    stage2: { status: 'pending', note: 'OPA + Rego swap (per Tier 6 #6.1) — keeps audit row schema unchanged' },
    stage3: { status: 'pending', note: 'Live policy reload (no service restart needed)' },
    next_action: 'Tier 6 #6.1: pip install opa-python; ship Rego rule files; drill the swap',
  },
  {
    ...
    stage2: { status: 'pending', note: 'Propose-only loop: Goal → Plan → Suggest → operator approval' },
    stage3: { status: 'pending', note: 'Full Goal→Plan→Execute→Evaluate→Improve loop with §42 boundaries' },
    next_action: 'Stage-2 wiring: PaperclipManager class with set_goal() + suggest_next_task() methods',
  },
  {
    name: 'OpenClaw A2A',
    category: 'layer-11',
    ...
    stage2: { status: 'pending', note: 'Real Dispatch RPC + per-pair circuit breaker' },
    stage3: { status: 'pending', note: 'Multi-agent task graph (parallel fan-out + reduce + HITL)' },
    next_action: 'Stage-2: implement DispatchService.Dispatch RPC (currently comment-only in proto)',
  },
  {
    ...
    stage2: { status: 'pending', note: 'Ollama-backed classifier (qwen2.5) with heuristic fallback' },
    stage3: { status: 'pending', note: 'Risk-tier escalation: high-risk → 2-of-3 council vote OR human approval' },
    next_action: 'Stage-2: swap heuristic body for an Ollama call inside agent_router.classify',
  },
  {
