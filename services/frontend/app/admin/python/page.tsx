...
const TOPICS: Topic[] = [
  ...
  { name: 'exceptions', level: 'core', blurb: 'raise / try / except / else / finally. Catch specific; never bare except without re-raise.', whereInRepo: 'libs/py/documind_core/exceptions.py — AppError hierarchy' },
  ...
  { name: 'inheritance', level: 'core', blurb: 'class Foo(Base): super().__init__(...). Watch the MRO when multiple bases.' },
  ...
  { name: 'generators', level: 'intermediate', blurb: 'def with yield — lazy iterator factory. Memory-efficient for big sequences.' },
  { name: 'decorators', level: 'intermediate', blurb: '@decorator wraps a function. functools.wraps preserves metadata. Used for retries, tracing, auth.', whereInRepo: 'documind_core retry/tracing decorators' },
  { name: 'context managers', level: 'intermediate', blurb: '`with` blocks. Implements __enter__/__exit__. contextlib.contextmanager turns a generator into one.' },
  ...
  // ---- advanced ----
  { name: 'async / await', level: 'advanced', blurb: 'Coroutine functions return awaitables. await suspends until done. Single-threaded concurrency.', whereInRepo: 'every service uses async FastAPI handlers' },
  { name: 'coroutines', level: 'advanced', blurb: 'Functions defined with `async def`. Don\'t run until awaited or scheduled.' },
  { name: 'event loop', level: 'advanced', blurb: 'asyncio.run() owns one. Schedules + drives coroutines + IO. Single thread by default.' },
  { name: 'asyncio task orchestration', level: 'advanced', blurb: 'asyncio.gather / create_task / wait. Cancellation propagates via CancelledError.', whereInRepo: 'services/inference-svc/app/routers — health/upstreams probes in parallel' },
  ...
  { name: 'GIL', level: 'advanced', blurb: 'Global Interpreter Lock. One thread runs Python bytecode at a time. Released for IO + C extensions.' },
  ...
  { name: 'Pydantic models', level: 'typing', blurb: 'Runtime validation. Field(...) for constraints. ClassVar to escape the field machinery.', whereInRepo: 'services/*/app/schemas/__init__.py' },
  // ---- backend ----
  { name: 'FastAPI patterns', level: 'backend', blurb: 'Router → Service → Repository. Depends() for DI. response_model on every endpoint.' },
  ...
  { name: 'retries / breakers / idempotency', level: 'backend', blurb: 'Retry with exponential backoff. CircuitBreaker around dependencies. Idempotency key for write ops.', whereInRepo: 'libs/py/documind_core/circuit_breaker.py + mcp/idempotency.py' },
  { name: 'structured logging', level: 'backend', blurb: 'JSON formatter; correlation_id in every log line. Never print() in prod.' },
  { name: 'tracing instrumentation', level: 'backend', blurb: 'OTel: instrument_fastapi/httpx/asyncpg/redis. Custom spans on critical workflows.' },
  { name: 'metrics emission', level: 'backend', blurb: 'prometheus_client Counter/Histogram/Gauge. Cardinality discipline: bound label sets.' },
  // ---- rag ----
