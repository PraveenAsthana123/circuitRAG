...
🧩 **Service**  ·  **Path:** `services/agent-orchestrator-svc`  ·  **Generated:** 2026-05-16 23:25 UTC
...
| go.mod | ❌ |
| package.json | ❌ |
| Top git contributors | `54	PraveenAsthana123` |
...
| 4 | Pagination on list endpoints | **10** | ✓ `offset` + `limit` Query params — detected at `app/main.py:303` |
| 5 | Caches bounded (LRU/TTL) | **10** | detected: redis |
| 6 | Async I/O where applicable | **10** | 140 async functions detected |
| 7 | Timeouts on all external calls | **10** | ✓ timeout= or asyncio.wait_for — detected at `app/agents.py:166` |
...
| 2 | Circuit breaker on external deps | **10** | ✓ CircuitBreaker wired — detected at `app/db_circuit_breaker.py:66` |
| 3 | Graceful degradation path | TBD | reviewer audit |
| 4 | Health probe (startup/liveness/readiness) | **10** | ✓ `/health` endpoint — detected at `app/main.py:224` |
...
| 3 | Structured JSON logs | **10** | ✓ structured logger — detected at `app/main.py:65` |
| 4 | correlation_id propagated everywhere | **10** | ✓ correlation_id used — detected at `app/main.py:403` |
