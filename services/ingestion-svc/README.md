...
🧩 **Service**  ·  **Path:** `services/ingestion-svc`  ·  **Generated:** 2026-05-16 23:25 UTC
...
| 4 | Pagination on list endpoints | **10** | ✓ `offset` + `limit` Query params — detected at `app/routers/documents.py:64` |
| 5 | Caches bounded (LRU/TTL) | **10** | detected: in-memory @lru_cache, redis |
| 6 | Async I/O where applicable | **10** | 71 async functions detected |
...
| 2 | Circuit breaker on external deps | **10** | ✓ CircuitBreaker wired — detected at `app/embedding/ollama_embedder.py:43` |
| 3 | Graceful degradation path | TBD | reviewer audit |
| 4 | Health probe (startup/liveness/readiness) | **10** | ✓ `/health` endpoint — detected at `app/routers/health.py:11` |
...
| 3 | Structured JSON logs | **10** | ✓ structured logger — detected at `app/main.py:58` |
| 4 | correlation_id propagated everywhere | **10** | ✓ correlation_id used — detected at `app/main.py:191` |
| 5 | Tracing (OTel) wired | TBD | — |
| 6 | Metrics exposed (RED: rate/errors/duration) | **10** | ✓ Prometheus instrumentation — detected at `app/services/poisoning_defense.py:28` |
...
| 2 | Test cases auto-parsed | **10** | 7 test functions |
...
| 10 | Sequence diagrams up-to-date | **10** | 7 endpoints diagrammed |
