...
- **`docs/architecture/adr/022-parallel-tool-convergent-work.md`** (ADR-022) — convergent-work pattern: autonomous-loop and parallel-tool independently arrive at byte-identical artifacts (observed 2x — ADR-021 itself + Phase 7RR drill bundle); detect via `git status` "nothing added to commit"; verify via `diff`; register parallel-tool's commit in the cadence ratchet; convergence is positive evidence that project conventions are sufficiently rigid
