...
| Phase-6X | **Agentic control-plane arc**. Added normalized persistence schema for project plan rows / task runs / approvals / memories (`007_agentic_project_memory.sql`), wired models + store + service persistence, exposed read APIs in `agent-orchestrator-svc`, added `/admin/agentic/control-plane` plus left-menu node and operator dashboard summary, and documented the surfaces in `docs/runbooks/agentic-control-plane.md` + cheatsheet + retrospective links. Drill coverage spans persistence, API, UI, dashboard summary, and full cross-surface chain: `drill_agentic_project_plan_persistence`, `drill_agentic_task_run_persistence`, `drill_agentic_approval_persistence`, `drill_agentic_memory_persistence`, `drill_agentic_control_plane_api`, `drill_agentic_control_plane_ui`, `drill_agentic_control_plane_chain`, `drill_admin_agentic_summary_panel`. | _this commit_ | implemented + drilled |
| Phase-6Y | **Monitoring/runtime truth arc**. Expanded `/admin/monitoring` from a small health page into a technical operations map with score/status, input/process/output, local stack inventory, tracking/login/observability surfaces, done/in-progress/pending boards, active/running service visibility, active agent task summary, and top resource consumers. Added frontend-owned local runtime route `/app-meta/runtime-status` that reads `docker compose ps --format json`, `docker stats --no-stream`, and `systemctl is-active ollama` to expose real local runtime truth. Observability hardening under the same arc now includes Grafana dashboard auto-provisioning, Prometheus rule loading, local Alertmanager, configurable shared-webhook receiver contract, node-exporter + cAdvisor for host/container metrics, and dashboard panels for those exporter metrics. Updated repo/service/frontend READMEs and runbooks (`README.md`, `services/agent-orchestrator-svc/README.md`, `services/frontend/README.md`, `docs/runbooks/agentic-control-plane.md`, `docs/runbooks/autonomous-loop-cheatsheet.md`, `docs/runbooks/council-telemetry.md`) so architecture docs now distinguish reference architecture vs live runtime truth and sidecar/council truth vs runtime/platform truth. Guardrails now include `mcp/tests/drill_admin_monitoring_runtime_surface.py`, `mcp/tests/drill_runtime_status_route.py`, `mcp/tests/drill_observability_stack_provisioning.py`, and `mcp/tests/drill_alertmanager_receiver_config.py`. | _this commit_ | implemented + drilled |
...
| 2026-04-29 | **Retroactive log** for Phase 7AA: extend Phase 1B grant to include `services/frontend/app/admin/sidecar/[eventId]/page.tsx` (per-event drill-down for sidecar rating surface) plus `services/frontend/app/api/v1/sidecar/events/[eventId]/...` write route. Operator authorized via the rapid-cadence "next" pattern that landed G-buckets G-1 through G-4 implicitly; parallel-tool's worktree included this route + drill_sidecar_rating_surface. `drill_section7_scope_log_mapping` caught the gap as REJECT (rule 1) on the Phase 7Z post-commit watcher run. This entry retroactively records the grant; the section7 drill's structural rewrite (this iteration) auto-extends as new sidecar/<segment>/page.tsx routes land. | **Granted retroactively** — `[eventId]/page.tsx` already in worktree; allowed paths under sidecar/ now: `page.tsx`, `deep/page.tsx`, `telemetry/page.tsx`, `[eventId]/page.tsx`. Write surface at `/api/v1/sidecar/events/[eventId]` similarly grandfathered. |
