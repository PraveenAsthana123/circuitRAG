...
| Phase-6Y | **Monitoring/runtime truth arc**. Expanded `/admin/monitoring` from a small health page into a technical operations map with score/status, input/process/output, local stack inventory, tracking/login/observability surfaces, done/in-progress/pending boards, active/running service visibility, active agent task summary, and top resource consumers. Added frontend-owned local runtime route `/app-meta/runtime-status` that reads `docker compose ps --format json`, `docker stats --no-stream`, and `systemctl is-active ollama` to expose real local runtime truth. Updated repo/service/frontend READMEs and runbooks (`README.md`, `services/agent-orchestrator-svc/README.md`, `services/frontend/README.md`, `docs/runbooks/agentic-control-plane.md`, `docs/runbooks/autonomous-loop-cheatsheet.md`, `docs/runbooks/council-telemetry.md`) so architecture docs now distinguish reference architecture vs live runtime truth and sidecar/council truth vs runtime/platform truth. First structural guardrail now exists: `mcp/tests/drill_admin_monitoring_runtime_surface.py`. | _this commit_ | implemented + drilled |
