...
| Phase-5V | `--prometheus + --weekly` per-week labels via `render_prometheus_weekly`; same metric names as single-window so `sum without (week)` rolls up cleanly; zero-pad PER WEEK that has data; weeks without data don't phantom-pad; unparseable rows surface as `week="unparseable"` | _this commit_ | 8 (6 negatives) |

**Cumulative:** 44 commits this session, 344 drill steps green across 42 sidecar/policy/pipeline/UI/ADR/migrate/bootstrap drills, **433 steps via the resource-aware runner** across all 58 tier-1 drills, 4 catalogued Ollama coder models locally installed (+ Kimi K2 documented as cloud tier).
