...
| 5.5 | **PR management subsystem** | 🟡 partial — auto-PR foundation ✅ DONE | `scripts/pr_management.py` — `find_batch_commits()` reads `git log origin/main..HEAD` + matches `auto-apply-*` tags; `build_pr_spec()` constructs 64K-cap PullRequestSpec with body citing every tag + §42/§51/§54 policy refs; `create_pr()` calls `gh pr create` only on `--confirm` (double-gate per §42). Subcommands: `preview` (dry-run; default), `create --confirm` (actual). Drilled 8/8: title 72-char cap; head pattern enforces git-ref chars; empty commit list raises; body cites tags + 3 policies + no Co-Authored-By. Auto-review (Tier-B Claude) + auto-merge gates ⏳ deferred to v2 (separate iter). |
...
| 5.7 | **Task management subsystem** | ✅ DONE | `scripts/task_manager.py` — Task Pydantic schema (id kebab-case, owner_type ∈ {human,agent}, status ∈ {pending/in_progress/blocked/done/cancelled}, priority ∈ {low/medium/high/urgent}, depends_on DAG-validated, tags). JSONL append-only at `.loop/tasks.jsonl`; latest-row-wins on read. CLI: `add / list / update / board` (Kanban shape). DAG cycle detection rejects self-cycle + arbitrary-depth cycles at add time. Drilled 8/8 with status Literal + extra='forbid' + cycle reject + uppercase-id reject + 3-row append + latest-row-wins. |
