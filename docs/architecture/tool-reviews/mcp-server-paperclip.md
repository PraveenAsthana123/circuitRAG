# `mcp/server_paperclip.py` — Brutal Tool Review
...
**Source:** `mcp/server_paperclip.py`
**Catalog:** `config/tool_catalog/paperclip.yaml`
...
| # | Dimension | Status | Note |
|---|---|---|---|
| 7 | Concurrency cap on probe / recovery | ✓ | Semaphore present |
| 8 | Required success threshold | ⚠ | no in-server breaker; relies on caller's circuit |
| 9 | Exponential backoff + jitter | ✓ | retry with backoff |
| 10 | Bulkhead / max-concurrent | ✓ | concurrency cap present |
...
| P1 (silent-degradation) | 17 | 2, 6, 8, 11, 12, 14, 19, 23, 24, 25, 26, 29, 32, 33, 34, 36, 39 |
...
| Architect | ✓ | C4 position locked via `config/tool_catalog/paperclip.yaml`; ADR via SDLC ADR set |
