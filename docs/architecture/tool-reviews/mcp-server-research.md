# `mcp/server_research.py` — Brutal Tool Review
...
**Source:** `mcp/server_research.py`
**Catalog:** `config/tool_catalog/research.yaml`
...
| 5 | Narrowed exception scope | ✓ | malformed HTML parser fallback catches `AssertionError` / `ValueError`, not broad `Exception` |
...
| 27 | OTel propagation | ✓ | `setup_server_otel(app, service_name="mcp-server-research")` + `/metrics` mounted |
...
| Architect | ✓ | C4 position locked via `config/tool_catalog/research.yaml`; ADR via SDLC ADR set |
...
> P0 blockers closed; P1 hardening remains before claiming full production-grade for this namespace.
