# `mcp/server_github_actions.py` — Brutal Tool Review
...
**Source:** `mcp/server_github_actions.py`
**Catalog:** `config/tool_catalog/github_actions.yaml`
...
|---|---|---|---|
| 1 | Per-call timeout | ⚠ | no explicit timeout — use HTTP client default |
...
| 6 | No silent fallback to fake data | ✓ | _live_or_stub returns available:False on missing creds |
...
| # | Dimension | Status | Note |
|---|---|---|---|
| 13 | Latency histogram | ✓ | OTel + Prom histogram via setup_server_otel |
| 14 | Success counter | ⚠ | no in-process counter; relies on OTel collector |
| 15 | Exception-class label | ✓ | log + span carry exception class |
...
| 26 | Per-tenant scope (§41.3) | ⚠ | verify tenant_id reaches downstream calls |
| 27 | OTel propagation | ✓ | OTel set up |
...
| 34 | Graceful shutdown | ⚠ | BACKLOG: add lifespan for graceful shutdown |
...
| 37 | Idempotency under retry | ✓ | idempotency-key honored |
...
| P1 (silent-degradation) | 20 | 1, 2, 7, 8, 9, 10, 11, 12, 14, 19, 23, 24, 25, 26, 29, 32, 33, 34, 36, 39 |
...
| Lens | Status | Gap |
|---|---|---|
| Developer | ✓ | Local-runnable; drilled |
| Architect | ✓ | C4 position locked via `config/tool_catalog/github_actions.yaml`; ADR via SDLC ADR set |
