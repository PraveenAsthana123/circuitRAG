...
| Event Bus | aiokafka producer/consumer wiring | ✅ shipped (iter-50..55) | All 5 FastAPI services publish CloudEvents-compliant events. `services/ingestion-svc` → document.lifecycle (existing; outbox pattern). `services/inference-svc` → query.generated.v1 (iter-50/51). `services/retrieval-svc` → query.retrieved.v1 (iter-53/54). `services/agent-orchestrator-svc` → agent.task.created.v1 (iter-53/54). `services/evaluation-svc` → eval.completed.v1 (iter-53/55). Lifespan + publish-point each independently drilled. Verify: `python3 mcp/tests/drill_kafka_fleet_lifespan.py` (7/7) + `python3 mcp/tests/drill_kafka_publish_points_fleet.py` (7/7 over 3 publish points). Operator opt-in via DOCUMIND_KAFKA_BOOTSTRAP. |
...
| Eval / Ragas | RAG-specific eval (faithfulness, relevance) | 🟡 partial — iter-35 scaffold + `ragas 0.4.3` installed (post-`install_all_oss_tools.sh --quick`); Stage-2 wire into eval_harness pending (status flips to ✅ when first real RAG eval lands and the drill empirically verifies a faithfulness score) | `python3 mcp/tests/drill_eval_engines_stage2.py` (`RAGAS_EVAL_ENABLED=1`) |
| Eval / Giskard | LLM red-team + bias scan | 🟡 partial — iter-37 scaffold + `giskard 2.19.1` installed (post-`install_all_oss_tools.sh --quick`); Stage-2 wire into eval_harness still pending | `python3 mcp/tests/drill_eval_lakera_giskard_scaffolds.py` (`GISKARD_SCAN_ENABLED=1`) |
| Eval / Lakera + Rebuff | Prompt-injection defense | ✅ shipped — iter-37 scaffold + `bad7b2d` Stage-2 runtime wire into `rag_inference.ask` + `rebuff 0.1.1` package installed (records signal; doesn't block on its own — defense-in-depth alongside regex `injection_detector`) | `python3 mcp/tests/drill_rebuff_detector_stage1.py` + `python3 mcp/tests/drill_rebuff_in_inference_stage2.py` (`REBUFF_ENABLED=1`) |
| Eval / DeepEval | Alternative RAG eval | 🟡 partial — `deepeval 3.9.9` installed but has known Python 3.12 compat issue (`module 'ssl' has no attribute 'wrap_socket'` — upstream bug); drill scaffold still works via offline-safe path | `python3 mcp/tests/drill_eval_engines_stage2.py` (`DEEPEVAL_ENABLED=1`) |
