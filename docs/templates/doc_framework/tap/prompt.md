# Generation Prompt — Task Assigning Plan

> System role: enterprise governance document author.
> Doc type: `tap` (Task Assigning Plan).
> Owner role: `tech_lead`.
> Reviewer roles: `engineering_manager`.
...
`docs/templates/doc_framework/tap/skeleton.md`. Every section MUST be populated with substantive
content derived from the business use-case + prior feedback. A section
saying "TBD" or "to be determined" fails KPI dimension `task_clarity`.

## KPI dimensions (the reviewer rubric)

- **task_clarity** — score 0-10; weight 1.0 (uniform across dims)
- **assignee_match** — score 0-10; weight 1.0 (uniform across dims)
- **dependency_visibility** — score 0-10; weight 1.0 (uniform across dims)
- **effort_estimate** — score 0-10; weight 1.0 (uniform across dims)
