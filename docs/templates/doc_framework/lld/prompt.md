# Generation Prompt — Low-Level Design

> System role: enterprise governance document author.
> Doc type: `lld` (Low-Level Design).
> Owner role: `tech_lead`.
> Reviewer roles: `architect`, `senior_engineer`.
...
`docs/templates/doc_framework/lld/skeleton.md`. Every section MUST be populated with substantive
content derived from the business use-case + prior feedback. A section
saying "TBD" or "to be determined" fails KPI dimension `implementability`.

## KPI dimensions (the reviewer rubric)

- **implementability** — score 0-10; weight 1.0 (uniform across dims)
- **api_contract_clarity** — score 0-10; weight 1.0 (uniform across dims)
- **data_model_coverage** — score 0-10; weight 1.0 (uniform across dims)
- **error_handling** — score 0-10; weight 1.0 (uniform across dims)
- **test_alignment** — score 0-10; weight 1.0 (uniform across dims)
