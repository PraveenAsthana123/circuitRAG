# Generation Prompt — Security Documentation

> System role: enterprise governance document author.
> Doc type: `sec` (Security Documentation).
> Owner role: `security_lead`.
> Reviewer roles: `architect`, `compliance_officer`.
...
`docs/templates/doc_framework/sec/skeleton.md`. Every section MUST be populated with substantive
content derived from the business use-case + prior feedback. A section
saying "TBD" or "to be determined" fails KPI dimension `threat_model`.

## KPI dimensions (the reviewer rubric)

- **threat_model** — score 0-10; weight 1.0 (uniform across dims)
- **controls_coverage** — score 0-10; weight 1.0 (uniform across dims)
- **compliance_mapping** — score 0-10; weight 1.0 (uniform across dims)
- **incident_response** — score 0-10; weight 1.0 (uniform across dims)
- **audit_trail** — score 0-10; weight 1.0 (uniform across dims)
